...
relative imports/Sass load roots -> resolver --------+-> WebStorm plugin
                                                    \-> VS Code extension
```
...
safe rename. Sass module graphs cover variables, mixins, functions, `@use`, and
`@forward`, including navigation, references, rename, auto-import completion,
and path normalization. Dynamic interpolation evaluation, ICSS, and CodeLens
remain deferred. WebStorm selector previews are
